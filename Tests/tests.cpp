...
///\details // This constructor takes a hwlib::i2c_bus_bit_banged_scl_sda i2c wich represnts the i2c bus,
            // const uint8_t chipaddres wich represents the RTC addres, unsigned char Date[] witch represents the a buffer for the date,
            ...
            // Tests object(hwlib::i2c_bus_bit_banged_scl_sda, a_chipaddres); The rest of the values in the constructor have predefined default values
Tests::Tests(hwlib::i2c_bus_bit_banged_scl_sda & i2c, const uint8_t chipaddres, unsigned char Date[],
    int testDay, int testMonth, int testYear,unsigned char Bin, 
    unsigned char DecToBin,int Dec,int BinToDec):
    DS3231(i2c,chipaddres),i2c(i2c),chipaddres(chipaddres),testDay(testDay),testMonth(testMonth),testYear(testYear), Bin(Bin),DecToBin(DecToBin),Dec(Dec),BinToDec(BinToDec){}
...
void Tests::PrintTestResults(){
    ...
    hwlib::cout<<"Test write/read to ds3231: "<<Test_write_read()<<hwlib::endl;
